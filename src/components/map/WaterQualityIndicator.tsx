...
const WaterQualityIndicator = () => {
  const map = useMap();
  const [qualityIndex, setQualityIndex] = useState(72);
  const [showDetails, setShowDetails] = useState(false);
  
  // Simulate water quality data updates
  useEffect(() => {
  ...
  return (
    <div className="absolute top-24 left-4 z-[2000]">
      <motion.div 
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.5 }}
        className="bg-card/80 backdrop-blur-md p-2 rounded-lg shadow-lg border border-white/10 w-44"
      >
        <div className="flex flex-col gap-2">
          ...
          <motion.div 
            className="relative h-2.5 w-full bg-secondary/80 rounded-full overflow-hidden"
          ...
          <motion.button
            ...
            className="text-xs flex items-center justify-center gap-1 py-1.5 px-2 rounded-md bg-secondary/50 hover:bg-secondary/80 transition-colors mt-1"
          >
          ...
          <AnimatePresence>
            {showDetails && (
              ...
              >
                <div className="text-xs space-y-2 mt-1 border-t border-white/10 pt-2">
                  <div className="flex items-center gap-1.5">
                    <span className="text-xs text-foreground/70">Regions:</span>
                    <div className="flex flex-wrap gap-1">
                      {getRegions(qualityIndex).map((region) => (
                        <span key={region} className="text-[10px] font-medium px-1.5 py-0.5 bg-ocean/20 rounded-full text-ocean-300">
                          {region}
                        </span>
                      ))}
                    </div>
                  ...
                  <div className="flex items-start gap-1.5">
                    <Waves className="w-3.5 h-3.5 text-ocean mt-0.5" />
                    <div>
                      <div className="text-[10px] text-foreground/70">Updated every 10s with satellite data</div>
