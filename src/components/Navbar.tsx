...
const Navbar = () => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-300 ${
      isScrolled ? 'py-2 glass-container backdrop-blur border-b border-white/5' : 'py-4 bg-transparent'
    }`}>
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center">
          <Link to="/" className="flex items-center space-x-2 group">
            <span className="relative">
              <Droplet className="h-7 w-7 text-ocean" />
            ...
            <span className="font-bold text-xl tracking-tight ocean-text">Sea Guardian</span>
          </Link>
